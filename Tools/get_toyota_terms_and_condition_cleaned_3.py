...
def get_toyota_terms_conditions(
    ...
    try:
        ...
        # Remove _path and clean HTML from conditions
        cleaned_terms = _clean_terms_data(filtered_terms)
        ...
        # Convert to JSON string and back to dict to ensure JSON serializable
...
def _clean_terms_data(terms_data: List[Dict]) -> List[Dict]:
    """Remove _path key and HTML tags from terms data."""
    cleaned_terms = []
    for term in terms_data:
        # Create a copy without _path key
        cleaned_term = {key: value for key, value in term.items() if key != '_path'}
        
        # Clean HTML from conditions if they exist
        if 'conditions' in cleaned_term and cleaned_term['conditions']:
            cleaned_conditions = []
            for condition in cleaned_term['conditions']:
                # Use plaintext version instead of HTML
                if 'plaintext' in condition:
                    cleaned_condition = {
                        'text': condition['plaintext'].strip().replace('\n\n', '\n')
                    }
                    cleaned_conditions.append(cleaned_condition)
            cleaned_term['conditions'] = cleaned_conditions
